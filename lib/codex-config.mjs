...
import { ConflictError, normalizeUrl } from "./validation.mjs";
...
export function createCodexConfig({ dir, dirSource, revisionKey }) {
  ...
  function publicState() {
    ...
    return {
      ...
      // The names actually written last time. Reported so the UI can show
      // the real `codex --profile` commands instead of predicting them from
      // a second copy of the slug rules.
      generatedProfiles: loaded.store.generatedProfiles,
