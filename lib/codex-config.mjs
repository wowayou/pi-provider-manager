...
export function createCodexConfig({ dir, dirSource, revisionKey }) {
  ...
  // Removing several providers in one commit, so a shifting revision cannot 409
  // the second removal onward and the set is all-or-nothing under one write.
  // Same invariant as the single delete, extended to a set: if the live
  // provider is among them, the replacement that takes over the owned table
  // must be one of the survivors — the store refuses a replacement that is
  // itself being deleted. Unlike Pi there is no keep-credential option: a
  // Codex credential lives inside its store entry, so removing the entry
  ...
  function deleteProvidersBulk(payload) {
    ...
    if (!rawIds || rawIds.length === 0) throw new Error("请选择要删除的供应商。");
    ...
    for (const id of providerIds) {
      if (!Object.hasOwn(store.providers, id)) throw new Error("要删除的供应商不存在。");
    }

    const deleting = new Set(providerIds);
    let auth = loaded.auth;
    let writeAuth = false;
    const wasActive = deleting.has(resolved.activeProviderId);
    if (wasActive) {
      const replacementId = String(payload.replacementProviderId || "").trim();
      if (deleting.has(replacementId) || !Object.hasOwn(store.providers, replacementId)) {
        throw new Error("删除当前生效的供应商前，请从保留下来的供应商里选择一个接替它。");
      }
      const provider = applyActive(loaded.document, store, replacementId);
      auth = applyAuth(auth, provider);
      writeAuth = needsAuthWrite(provider);
    ...
    for (const id of providerIds) {
      ...
      if (store.activeProviderId === id) store.activeProviderId = "";
    }
    commit(revision, loaded.document, auth, store, writeAuth);
    return { providerIds, replacedActive: wasActive };
  }
