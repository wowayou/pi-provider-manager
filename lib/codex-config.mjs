...
export function createCodexConfig({ dir, dirSource, revisionKey }) {
  ...
  function applyActive(document, store, providerId) {
    ...
    if (provider.requiresAuth && !provider.credential?.key) {
  ...
  function applyAuth(auth, provider) {
    ...
    // auth.json — the user may have a ChatGPT login sitting in it.
    if (!provider.requiresAuth) return auth;
  ...
  function saveProvider(payload) {
    ...
    if (shouldActivate) {
      ...
      writeAuth = provider.requiresAuth;
  ...
  function activate(payload) {
    ...
    commit(revision, loaded.document, auth, store, provider.requiresAuth);
    return { providerId };
  }
  ...
  function saveSettings(payload) {
    ...
    let auth = loaded.auth;
    let writeAuth = false;
    const activeId = String(payload.activeProviderId || store.activeProviderId || "");
    if (activeId && Object.hasOwn(store.providers, activeId)) {
      const provider = applyActive(loaded.document, store, activeId);
      auth = applyAuth(auth, provider);
      writeAuth = provider.requiresAuth;
    }
    commit(revision, loaded.document, auth, store, writeAuth);
