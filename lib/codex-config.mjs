  ...
  function resolve({ document, auth, store }) {
    ...
    if (!activeProviderId) {
      ...
      providers[adoptedProviderId] = {
        ...
        // `upstream` records what the user chose in the wizard. config.toml
        // carries no such declaration, so an adopted provider makes no claim:
        // a loopback base URL may be a translation bridge, some other local
        // proxy, or a local model server. Anything the UI needs to say about a
        // local address it derives from the URL instead.
        upstream: "direct",
