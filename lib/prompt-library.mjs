    ...
    for (const slot of slotList) {
      ...
      if (onDisk !== null) {
        ...
        if (match) {
          ...
          // Present but empty, matching no stored document: there is nothing to
          // adopt. An empty file carries no author-written content, so it is not
          // taken over as "现有内容" — doing so stamped a misleading, empty
          // "已接管/生效中" entry onto a brand-new setup. Treated like an absent
          // file: nothing is live and the user starts from 新建提示词. (A stored
          // document whose own text is "" still matches above and stays live.)
          activeId = "";
        }
      } else {
