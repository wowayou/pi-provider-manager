...
// A transport failure arrives as `TypeError: fetch failed`, with the actual
// reason hidden on `cause`. That is what a Windows user saw when this host's
// security software reset every connection to GitHub's object storage while the
// same download from WSL on the same machine worked: a message that named
// neither the host nor the reason, and so read as a bug in the manager. Name
// both. The host distinguishes "GitHub is unreachable" from "the API answered
// and the download did not", and the code is what turns a support question into
// a proxy or firewall check.
export function transportFailure(url, error) {
  let host = "下载地址";
  try {
    host = new URL(url).host;
  } catch {}
  // undici reports the errno itself (`ECONNRESET`) in some cases and its own
  // wrapper (`UND_ERR_SOCKET`) in others, so prefer the code and fall back to
  // whatever text there is. `fetch failed` alone is the one thing the reader
  // must not be left with.
  const cause = error?.cause;
  const reason = cause?.code || cause?.message || error?.message || "未知原因";
  return new Error(`无法连接 ${host}：${reason}。请检查网络、代理或本机安全软件是否拦截了这个连接。`);
}
...
async function defaultFetchBinary(url, maxBytes = MAX_ARCHIVE_BYTES) {
  ...
  for (;;) {
    ...
    try {
      ...
      // A connection cut halfway through is the same class of failure as one
      // refused outright, and just as unreadable when it is not translated.
      throw transportFailure(url, error);
    }
    if (done) break;
