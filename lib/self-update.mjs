...
function transportFailure(url, error) {
  ...
  // Both halves are worth carrying: undici sometimes reports the errno itself
  // (`ECONNRESET`) and sometimes its own wrapper (`UND_ERR_SOCKET`), and the
  // message underneath is what says which. `fetch failed` on its own is the one
  // thing that must not be what the reader sees.
  const cause = error?.cause;
  const code = cause?.code || error?.code || "";
  const detail = typeof cause?.message === "string" && cause.message !== "" ? cause.message : "";
  const reason = [code, detail && detail !== code ? `（${detail}）` : ""].filter(Boolean).join("")
    || error?.message
    || "未知原因";
