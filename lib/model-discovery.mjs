...
// Pi's own rule for Anthropic: an OAuth token travels as a Bearer credential,
// an API key as `x-api-key`.
function anthropicAuth(key) {
  return key.includes("sk-ant-oat") ? { authorization: `Bearer ${key}` } : { "x-api-key": key };
}
...
export function discoveryRequest(api, baseUrl, key) {
  ...
  if (api === "anthropic-messages") {
    return {
      ...
      headers: { ...anthropicAuth(credential), "anthropic-version": ANTHROPIC_VERSION, accept: "application/json" },
    };
  }
