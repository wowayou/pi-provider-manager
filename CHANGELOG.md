...
- Added `npm run test:codex-real`, which runs the installed `codex` binary against a manager-generated config and asserts `codex doctor` loaded it. It skips itself where Codex is absent, so it is the first thing to run after a Codex upgrade.
