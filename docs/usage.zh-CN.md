...
- **获取模型**：点「获取模型」，管理器在本机服务端用保存时会用的那把 key（第二步刚填的，或已保存的）向网关
  ...
  Messages 网关请求 `<地址>/v1/models`，OpenAI / Gemini 网关请求 `<地址>/models` —— 所以列表能拿到，
  对话地址基本也就对了。key 不回传浏览器，网关的错误只转述状态（401 是 key 不对，404 是网关没这个接口，
  改手填），重定向会直接拒绝以免 key 被转走。已在列表里的 ID 会显示但不能重复导入。
...
同一张卡片上还有「检查更新」。**只有按下它才会联网**：向 `api.github.com` 查询本项目的最新
...
- **git checkout**：「拉取并构建 x.y.z」。顺序是 `git fetch` → `git merge --ff-only` →
...
- **归档安装**：「下载 x.y.z 到相邻目录」。新版本解包到当前目录的**相邻位置**
...
**一次删除多家（仅 Pi）**：侧栏标题旁的「选择」进入多选模式，每行左侧出现勾选框，点行即勾选；
...
当前默认项时，要求从**保留下来的**供应商里选替代供应商和模型。同样带 revision 校验，磁盘在你操作
期间被改过就返回 409 且不写盘。切换侧栏到 Codex 会自动退出多选模式。
