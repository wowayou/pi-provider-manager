...
侧栏「我的供应商 / API 网关」→ 添加供应商，三步。列表里每一行会显示它有多少个模型、用哪种
协议、凭据是否已配好；**Pi 当前默认的那一家标「默认」**，一眼能看出实际在用哪一个。

每行右侧还有一个「更多操作」菜单（鼠标移上去或键盘聚焦到那一行时出现），**复制供应商**和
**删除供应商**都能直接在这里点，不必先把这家打开进向导。标题旁的「选择」进入多选模式，用来一次
删除多家 —— 见[第九节](#九删除以及不会被你覆盖掉的东西)。
...
展开本步底部的「高级兼容设置」可以填写供应商级 `User-Agent`。它会作用于这个供应商下的模型，
但某个模型或扩展配置也可能覆盖它，所以保存后的成功页会再次提醒这一点。通常无需填写；只在网关
明确要求特定 UA 时使用。输入框只接受普通的可打印 ASCII 字面量（最多 512 字节），不会执行 Pi
支持的动态表达式。已有但无法安全读取的外部/动态值只显示「由外部配置」：保存其它设置会保留它，
只有输入或点击清除才会明确替换。四个「快速填充」值只是来自公开客户端约定的填写示例，不保证任何
网关接受，也不会作为独立 preset 保存。

同一面板里还有**模型级 `Anthropic Beta 请求头`**。少数中转站仍要求 `context-1m-2025-08-07` 之类的 beta token
才放行 1M 上下文；Zed 里叫 `extra_beta_headers`，在 Pi 里就是模型的 `headers["anthropic-beta"]`。选一个模型，
填逗号分隔的 token，保存即写入。任何协议的模型都能填：Pi 对每种协议都会把模型的 `headers` 原样发出，只是
非 Anthropic 网关多半会忽略它，面板会提示这一点，但不会拦你 —— 早先的版本拦过，结果把「往 Anthropic 协议的
中转站里加一个 gpt 模型并给它换协议」这种正常操作也拦住了。真正要注意的是：**Pi 会用你填的列表
替换它自己自动加的 beta 列表**（工具流式、思考等），所以只在网关明确要求时填，管理器不会按 `[1M]` 后缀或
容量替你猜。已有但无法安全读取的值显示「外部配置」，只有输入或清除才会替换。
...
- **获取模型**：点「获取模型」，管理器在本机服务端用保存时会用的那把 key（第二步刚填的，或已保存的）向网关
  ...
  网关的错误只转述状态（401 表示认证被拒绝；404 时可先修改清单路径重试，仍不支持再手填），
  重定向会直接拒绝以免 key 被转走。已在列表里的 ID 会显示但不能重复导入。
- **模型列表路径可以自己填**：不是每个网关都把清单放在 Pi 推出来的那个地址。弹窗里有「模型列表路径」，
  留空就用协议默认值，填了就用你填的，按回车或点「用这个路径重试」即可，弹窗顶部始终显示这次实际请求的
  完整地址。路径**相对于 API 地址**，唯一的限制是**必须同源**：换主机、换端口、降级成 http、或用
  `user@` 这类写法都会被拒（那会把 key 发去别处）。允许跳出 API 地址所在的路径 —— DeepSeek 的对话地址是
  `https://api.deepseek.com/anthropic`，而它的清单在 `/v1/models`，实测填这个才拿得到。
- **批量添加**：把一串模型 ID 粘进「批量添加模型 ID」，一次生成多行。长清单不用一行行敲。
- **全部用安全值 / 这一行用安全值**：把上下文容量与最大输出改成安全默认值，**可撤销**。不确定
  某个网关的真实上限时先用这个，能起来再调。「这一行用安全值」在鼠标悬停或键盘焦点进入那一行时出现。
...
- **删除保护**：每一行都要先点一次查看后果，再点一次删除，并可撤销。不能单独删掉供应商的唯一模型；
  提示会引导你添加替代模型或删除整个供应商。删除 Pi 当前默认模型后，保存时还必须指定有效替代模型。
- **复制供应商**：多数新网关只是换个地址和 key，模型清单和兼容设置照搬。点「复制」会以当前供应商
  为模板开一份新草稿：ID 自动取一个没被占用的 `<原 ID>-copy`，模型行、思考档位、兼容开关全部
  带过来，然后停在凭据那一步等你填新 key。侧栏行内菜单里的「复制供应商」是同一个动作，区别只是
  不用先把这家打开。**凭据不会跟着复制** —— 已保存的 key 不回传浏览器，
  复制出来的这份也就没有 key 可用，必须自己填一个（或选「复用已有凭据」，这时不会动原供应商的
  那一条）。Codex 侧同样有这个按钮；用了托管桥的供应商，桥的上游地址属于配置会带过来，桥持有的
  上游 key 属于机密不会。
...
是哪个版本。启动器（`pi-provider-manager-ui`）复用已在运行的实例时也会打印同样的信息，并给出
`kill <pid> && …` 的重启命令。

卡片底部有一个按钮直接做这件事：有待应用的版本时它是「重启以应用 x.y.z」，没有时是「重启本地
服务」。它的顺序是先停止监听、再用磁盘上的文件起一个新进程、等到**另一个** pid 在端口上应答之后
当前进程才退出；新进程起不来（升级包坏了、文件只写了一半）就把端口收回来，并把原因显示在这张卡
片上——所以一次失败的升级不会让你既没有管理器也没有说明。接管成功后页面会自己刷新。

按钮只替换本管理器进程：已经在跑的 LiteLLM 桥是独立进程，不受影响；Pi 和 Codex 的会话本来就不
经过它。只有当你在这一页真的改过设置还没保存时，它才会先要一次确认——「有 N 项默认值还没写入
settings.json」不算未保存的修改，重启也不会让它变少。

### 检查更新

同一张卡片上还有「检查更新」。**只有按下它才会查询远端版本**：向 `api.github.com` 查询本项目的最新
Release。启动、构建、打开页面都不外联，也没有任何定时检查——这一条写在
[docs/compatibility.md](compatibility.md) 里，是产品约束而不是实现细节。匿名调用有频率限制
（每小时每 IP 60 次），撞上了会直接告诉你。

更新方式取决于安装形态：

- **git checkout**：拉取并构建当前分支的上游；即使 Release 版本号未变，也能更新尚未发版的提交。顺序是 `git fetch` → `git merge --ff-only` →
  只有 `package-lock.json` / `package.json` 变了才 `npm ci --ignore-scripts` → `npm run build`。
  任何一步失败就停在那一步，并把输出的最后几行显示出来；构建没成功绝不会让你去重启，因为那正好
  是「源码比 bundle 新」的错配。工作区不干净、分支没有远端、或者不是 git 仓库时，它直接拒绝并
  说明原因，不会替你 stash 或者 merge。
- **归档安装**：有新 Release 时显示「下载 x.y.z 到相邻目录」。新版本解包到当前目录的**相邻位置**
  （`pi-provider-manager-vX.Y.Z/`），当前安装一个字节都不动，下载的压缩包用完即删。完成后运行新
  目录里的启动器；确认没问题再删旧目录。就地替换一个正在运行的目录这件事本程序不做。

这一步只改磁盘。改完之后「管理器版本」还是旧的（进程没换），卡片会提示磁盘上已经是新版本，用上面
说的重启按钮才真正生效——两件事分开，是为了让你在替换正在对话的这个进程之前，先看一眼结果。

如果拉取成功但构建失败（或者你自己 `git pull` 之后没构建），源码就比 `dist/client` 新。这种状态下
重启只会让新的服务端配上旧界面，所以**重启按钮会停用并说明原因**，`POST /api/restart` 也会直接返回
`409`——不只是把按钮藏起来，因为发请求的页面可能比这条规则还旧。跑一次 `npm run build` 就恢复，
不需要重启。
...
三步向导：**「接入方式」→「填写凭据」→「确认模型」**。侧栏里**当前生效的那一家标「生效中」**——
Codex 同一时刻只有一个供应商在用，所以这里只有一个条目标它。
...
**删除 Pi 供应商**会明确说清影响范围，三种措辞对应三种情况：
...
**一次删除多家（Pi 和 Codex）**：侧栏标题旁的「选择」进入多选模式，每行左侧出现勾选框，点行即勾选；
底部出现一条操作条「全选 / 取消全选」「已选 N 个」「取消」「删除 (N)」。**全选只作用于当前筛选
出来的那些行** —— 上面有筛选条件时，选的是筛选结果，不是全部。点「删除选中」走的是同一套确认
对话框：一次写入删掉整批，**任一 ID 非法或不存在就整批拒绝、什么都不删**；待删集合里包含 Pi
当前默认项时，要求从**保留下来的**供应商里选替代供应商和模型。Codex 则要求为待删的生效供应商
选择一个保留的供应商接替，并删除所选条目的地址、模型和保存的 key，没有保留 key 选项。两边各用
自己的 revision 校验，磁盘在你操作期间被改过就返回 409 且不写盘。切换 Pi / Codex 会退出多选模式。
