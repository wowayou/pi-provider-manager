...
- 2026-09-17, Pi `0.85.1`, manager `0.3.16` + unreleased: `npm run test:pi-real` ran the installed `pi` against a loopback Anthropic stand-in with an isolated `PI_CODING_AGENT_DIR`, a dummy key, and every discovery switched off. Measured: a model saved with `anthropicBeta: "context-1m-2025-08-07, context-1m-2025-08-07, prompt-caching-2024-07-31"` reached the wire as `context-1m-2025-08-07,prompt-caching-2024-07-31` (deduplicated by the manager; Pi re-joins without spaces); the sibling model in the same provider sent no `anthropic-beta` at all in that configuration (`--thinking off --no-tools`, so Pi had no automatic betas to send); a `claude-big[1M]` ID was sent as-is with `contextWindow=1000000`/`maxTokens=128000`; clearing the override removed the header. The key travelled as `x-api-key`. 1 passed, 0 skipped.
- Same day, the second wire case — the one the paragraph above could not answer. Six configurations were measured to find what gives Pi an automatic beta list at all, and it is thinking, not tools: `--thinking high` made the sibling model send `interleaved-thinking-2025-05-14`, while `--thinking off` sent nothing with tools on or off. With that list in play, the model carrying `context-1m-2025-08-07, prompt-caching-2024-07-31` sent exactly those two tokens and **not** `interleaved-thinking-2025-05-14`, and clearing the override handed the same model its automatic list back. So the override replaces Pi's list rather than joining it — measured on the wire, not read off `getBetaFeatures`, and the concrete reason an inferred header would silently drop the thinking beta. `tests/pi-anthropic-beta-real.test.mjs` asserts the pair and fails loudly if thinking ever stops producing an automatic list, which is what would make the test vacuous. 2 passed, 0 skipped.
- Same day: `npm run test:server` 68 passed, 0 skipped (includes the discovery endpoint against a loopback gateway: stored/typed/migrated credentials, refusals before any request leaves, redirect refused, 401/404/HTML/unreachable explained, Host and Content-Type guards). `npm run test:ui` 13 passed on Node `24.18.0`, and 13 passed again on Node `22.22.2` (includes the 获取模型 dialog importing a listed model with the fixture's stored key and reporting an unreachable gateway with a retry, and the contrast audit, whose only exempt finding is the disabled button recorded above).
