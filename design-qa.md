...
- **Known demo gaps left open, recorded rather than hidden:** the prompt save/activate/delete paths and the two provider-delete dialogs still post to the real API under `?demo=1`. They fail safely (a refusal, not a wrong write), and this round did not widen further.
