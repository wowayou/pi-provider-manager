...
- A 409's corrective action (重新读取, a page reload) lives in the persistent error banner as well as in the toast that first reports it — the toast expires after seven seconds, the banner does not. The banner button renders only while the conflict flag from the last request error is live, and every save clears that flag at entry.
