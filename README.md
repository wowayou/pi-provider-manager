...
- **Large catalog UX** — sticky model header, internal scrolling, bulk model-ID import, a 获取模型 dialog that lists the gateway's own catalogue as checkboxes (fetched server-side with the credential a save would use, never returned to the browser), and warnings when `-max`/`-xhigh` may be thinking levels rather than real model IDs.
- **Per-model `anthropic-beta` header** — for gateways that still ask for `context-1m-2025-08-07` or another beta token: literal, validated, `anthropic-messages` models only, with the same none/literal/external contract as the User-Agent. `npm run test:pi-real` runs the installed Pi against a loopback gateway to prove what reaches the wire.
